...
#undef _LA_
    ...
    subroutine ommp_df_compute_induced_dipoles(s, solver, matv, add_mm_field, &
                                                add_nuclei_field, qm_helper, &
                                                exclude_df_field)
        !! Compute the induced dipoles on polarizable sites due to the
        !! electric field generated by fitted charges. Mirrors
        !! ommp_set_external_field but uses df%E_q2p as the external field.
        !!
        !! add_nuclei_field (default .true.): whether to add the nuclear electric
        !!   field to the fitted-charge field. If .true., either qm_helper or
        !!   nuc_field must be provided (but not both).
        !! qm_helper: pointer to a qm_helper object used to compute the nuclear
        !!   field. The qm_helper must have already been prepared with coordinates.
        !! nuc_field: direct nuclear field array (3 x n_pol) to use instead of
        !!   computing from qm_helper.
        !! exclude_df_field (default .false.): if .true., the electric field from
        !!   fitted charges (df%E_q2p) is NOT added to the external field ef.

        use mod_polarization, only: polarization
        use mod_electrostatics, only: prepare_polelec
        use mod_io, only: fatal_error
        use mod_density_fit, only: df_e_field_to_pol
        use mod_memory, only: mallocate, mfree

        implicit none

        type(ommp_system), intent(inout), target :: s
        integer(ommp_integer), intent(in) :: solver
        integer(ommp_integer), intent(in) :: matv
        logical, intent(in), optional :: add_mm_field
        logical, intent(in), optional :: add_nuclei_field
        type(ommp_qm_helper), pointer, optional :: qm_helper
        logical, intent(in), optional :: exclude_df_field
        ...
        !! Determine nuclear field behaviour
        if(present(add_nuclei_field)) then
            do_nuc_f = add_nuclei_field
        else
            do_nuc_f = .true.
        end if

        !! Determine whether to add the fitted-charge electric field
        if(present(exclude_df_field)) then
            do_add_df_f = .not. exclude_df_field
        else
            do_add_df_f = .true.
        end if

        !! Trigger E_q2p computation (lazy)
        call df_e_field_to_pol(s%df, eel)
        call mallocate('ommp_df_compute_induced_dipoles [ef]', &
                        3_ommp_integer, eel%pol_atoms, eel%n_ipd, ef)
        ef = 0.0

        !! Allocate extended external field if needed
        if(do_nuc_f) then
            if(present(qm_helper)) then
                !! Ensure E_n2p is computed in qm_helper
                if(.not. qm_helper%E_n2p_done) then
                    call ommp_prepare_qm_ele_ene(s, qm_helper)
                end if
                    do i=1, eel%n_ipd
        ...
        if(do_mm_f) then
            call prepare_polelec(eel)
            do i=1, eel%n_ipd
                ef(:,:,i) = ef(:,:,i) + eel%e_m2d(:,:,i)
                if(do_add_df_f) ef(:,:,i) = ef(:,:,i) + s%df%E_q2p
            end do
            call polarization(s, ef, solver)
        else
            call mallocate('ommp_df_compute_induced_dipoles [ef]', &
                           3_ommp_integer, eel%pol_atoms, eel%n_ipd, ef)
            if(do_add_df_f) ef(:,:,1) = ef(:,:,1) + s%df%E_q2p
            call polarization(s, ef, solver, matv, [.true., .false.])
        end if


        call mfree('ommp_df_compute_induced_dipoles [ef]', ef)
    end subroutine ommp_df_compute_induced_dipoles

    subroutine ommp_df_get_e_field_pol_ene(s, ene)
        !! Get the polarization energy from fitted-charge electric field.
        !! Triggers E_q2p computation and energy evaluation on first access.

        use mod_io, only: fatal_error
        use mod_density_fit, only: df_e_field_pol_ene
