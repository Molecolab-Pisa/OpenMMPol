    ...
    with open(fname, 'r') as f:
        for l in f:
            if l.startswith('[OMMP]') and not OMMP_out:
                ...
                OMMP_out = True
            if OMMP_out and l.startswith("[OMMP]  [TEST-HES] Hess "):
                tnames += [l.split()[3]]
            elif not OMMP_out and l.startswith("Hess "):
                tnames += [l.split()[1]]
    return tnames
    ...
    with open(fname, 'r') as f:
        for l in f:
            if l.startswith('[OMMP]') and pres == '':
                pres = "[OMMP]  [TEST-HES] "
            if l.startswith("{:s}Hess {:s}".format(pres, term)) and l[len("{:s}Hess {:s}".format(pres, term))] in ' \n':
                a = []
                l = next(f)
...
if len(tnames_a) == 0 or len(tnames_b) == 0:
    print("No Hessian terms to compare!")
    exit(1)
...
for tname in list(set(tnames_a) & set(tnames_b)):
    ...
    print("HESS {:s}".format(tname))
    for i in range(a.shape[0]):
        print("[{:5d}] {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f}".format(i, *a[i,:]))
        print("        {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f}".format(*b[i,:]))
        print("        {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f} {:10.4f}".format(*delta[i,:]))
    out = "Delta Max {:5s} {:6d}/{:1d} {:8.2g}({:8.2g}) {:8.2g}%({:8.2g}%) {}".format(tname, np.argmax(delta)//9, np.argmax(delta)%9, m, atol, mr, rtol*100, isok)
